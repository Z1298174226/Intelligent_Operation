...
public class TextKeyWord {
    ...
    public List<String> getKeyword(String title, String content) {
        ...
        CustomerDictionary.addDictionary();
        List<Term> termList = NLPTokenizer.segment(title + content);
        List<String> wordList = new LinkedList<String>();
        for (Term t : termList) {
            if (shouldInclude(t)) {
                if (!wordList.contains(t.word))
                    wordList.add(t.word);
            }
        }
...
//        }
    ...
    {
        return CoreStopWordDictionaryCustomer.shouldInclude(term);
    }
